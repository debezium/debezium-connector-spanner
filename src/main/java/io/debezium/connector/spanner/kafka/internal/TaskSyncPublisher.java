...
public class TaskSyncPublisher {
    ...
    public TaskSyncPublisher(
                             String taskUid,
                             String topic,
                             int syncEventPublisherWaitingTimeout,
    ...
    public void send(TaskSyncEvent taskSyncEvent) {
        if (bufferedPublisher == null) {
            ...
            LOGGER.debug(
                    "Buffering Sync Event, type: {}, timestamp: {}",
                    taskSyncEvent.getMessageType(),
                    taskSyncEvent.getMessageTimestamp());
    ...
    private void publishSyncEvent(TaskSyncEvent taskSyncEvent) {
        debug(
                LOGGER,
                "Send SyncEvent to Kafka topic, type: {}, timestamp: {}, event: {}",
                taskSyncEvent.getMessageType(),
                taskSyncEvent.getMessageTimestamp(),
                taskSyncEvent);
        ...
        try {
            ...
            close();
            errorHandler.accept(
                    new SpannerConnectorException("Error during publishing to the Sync Topic", e));
        }
    ...
    private boolean publishImmediately(TaskSyncEvent syncEvent) {
        return !(syncEvent.getMessageType() == null
                || syncEvent.getMessageType() == MessageTypeEnum.REGULAR);
    }
}
